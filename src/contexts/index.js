...
export function UserProvider({ children }) {
  const [role, setRole] = useState(() => localStorage.getItem('time_ms') || null);
  ...
  useEffect(() => {
    const onStorage = (e) => {
      ...
      if (e.key === 'time_ms') setRole(e.newValue);
    };
  ...
  useEffect(() => {
    ...
    if(role) localStorage.setItem('time_ms', role);
    else localStorage.removeItem('time_ms');
